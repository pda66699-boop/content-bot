...
TOPIC_PROFILES = {
    ...
    "оргструктура и роли": {
        ...
        "reason": "Причина обычно не в дисциплине. Причина в том, что компания выросла, а архитектура ролей и ответственности осталась на уровне 'договорились устно'.",
        "solution": "Здесь полезно начать с трёх вещей:\n– определить, какой результат должна держать каждая ключевая роль;\n– разделить зоны решений, а не только список задач;\n– закрепить, где сотрудник решает сам, а где эскалирует вопрос дальше.",
    ...
    "скрытые потери в операционке": {
        ...
        "reason": "Пока компания не видит процесс как систему с владельцем, входом, выходом и точкой контроля, она лечит видимые расходы и не трогает источник утечки.",
    ...
    "работа с причинами, а не симптомами": {
        ...
        "scene": [
            ...
            "Проблема обычно выглядит как частная: провалился срок, не сработал руководитель, буксует отдел. Но если посмотреть глубже, причина часто лежит в модели управления, которая уже не соответствует масштабу бизнеса.",
        ],
        "reason": "Когда компания вырастает, а способ управления остаётся прежним, собственник начинает постоянно работать с последствиями. Симптомы меняются, архитектурная причина — нет.",
        "solution": "Поэтому полезно разбирать не только сам сбой, но и три вопроса:\n– что именно сломалось в контуре управления;\n– на какой стадии бизнеса это стало нормой;\n– какая роль, процесс или ритм контроля здесь отсутствует.",
    ...
    "рост расходов и оптимизация затрат": {
        ...
        "scene": [
            ...
            "На поверхности кажется, что главная задача сейчас — просто сократить затраты. Но внутри компании деньги продолжают утекать в переделки, сбои процессов, хаотичные решения и перегруженную операционку.",
        ],
...
def infer_profile(theme: str) -> dict:
    ...
    return {
        ...
        "reason": "Проблема здесь редко в одном человеке или одном инструменте. Чаще всего не хватает архитектуры: роли, процесса, понятного результата и ритма контроля.",
...
def build_reason(profile: dict, angle: str, variant: int) -> str:
    base = profile["reason"]
    if variant % 3 == 1:
        return f"Если смотреть глубже, дело не в поверхности. {base}"
    if variant % 3 == 2:
        return f"Если разбирать это по сути, {base[0].lower() + base[1:]}"
    return base
...
def tune_scene_with_references(base_scene: str, reference_patterns: dict, variant: int) -> str:
    ...
    if body_mode == "diagnostic" and variant % 3 == 2:
        return f"Если смотреть на это не эмоционально, а управленчески, картина обычно выглядит так.\n\n{base_scene}"
    return base_scene
...
def build_stage_diagnostic_draft(context: DraftContext, variant: int) -> str:
    ...
    prefaces = [
        ...
        "Снаружи всё выглядит просто: люди уходят, руководители не тянут, между отделами бардак. Но если смотреть глубже, это часто не проблема людей, а проблема того, как выстроен сам найм и ввод человека в работу.",
    ...
    effects = [
        "После такой диагностики становится понятно, что именно чинить: найм, адаптацию, руководителей, профиль должности, роли или саму архитектуру компании.\n\nИ это уже системная работа, а не борьба с симптомами.",
    ...
    cta_options = [
        "Если узнаёшь в этом свой бизнес, можно быстро проверить, где у тебя ломается найм, адаптация или сама конструкция управления — для этого есть короткая диагностика в @adizesbizbot.",
...
def apply_stop_word_guard(text: str, stop_words: dict) -> str:
    cleaned = text
    replacements = {
        "главный вопрос здесь": "важнее другое",
        "ключевой момент здесь": "важнее другое",
        "фокус здесь": "смысл",
        "если смотреть глубже": "Если копнуть глубже",
        "если разбирать это по сути": "Если разбирать это трезво",
        "на поверхности кажется": "Сначала может казаться",
        "обычно это выглядит": "Чаще всего это выглядит",
        "в такой ситуации важно": "В такой ситуации лучше",
        "поэтому здесь полезно": "Здесь полезно",
        "тогда меняется и результат": "Тогда и результат становится другим",
        "после этого зона перестает держаться": "После этого зона уже не держится",
        "после этого эта зона перестает держаться": "После этого эта зона уже не держится",
    }
