...
REPLACEMENTS = [
    (
        "Если смотреть практично, главный вопрос здесь такой:",
    ...
    (
        "После этого зона перестаёт держаться на ручном включении.",
        "После этого эта зона перестаёт держаться только на ручном включении владельца.",
    ),
    (
        "Появляется порядок, снижаются потери и становится проще принимать решения без постоянного тушения локальных сбоев.",
...
def remove_templatey_phrases(text: str) -> str:
    ...
    text = text.replace(
        "Вопрос здесь обычно один: что именно в этой зоне у тебя до сих пор держится не на системе, а на ручном включении?",
        "Обычно в этот момент и становится видно, что проблема держится не на системе, а на ручном включении собственника.",
    )
    text = text.replace(
        "Если хочешь, позже могу разобрать это на конкретной управленческой сцене.",
